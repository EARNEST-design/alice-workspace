...
def model(height="0.62"):
    ...
    return URDF.from_xml_string(xacro.process_file(str(path), mappings={"height_m": height}).toxml())
...
def test_tree_covers_all_links_without_duplicate_names():
    ...
    assert len({l.name for l in robot.links}) == len(robot.links)
...
def test_motor_map_preserves_observed_channels_and_excludes_disconnected_output():
    ...
    assert known == {0: "neck_rotation", 1: "head_tilt", 2: "face_pitch",
                     3: "lower_eyelids", 4: "upper_eyelids", 5: "forehead_frown",
                     6: "mouth_open", 8: "right_eye_horizontal", 9: "left_mouth_corner",
                     10: "left_eye_horizontal", 11: "right_mouth_corner"}
    ...
    mapped = {r["joint"] for group in ("head_actuators", "body_actuators") for r in mapping[group]}
    independent = {j.name for j in robot.joints if j.type != "fixed" and j.mimic is None}
...
def test_paired_eyelids_mimic_their_single_actuators():
    ...
    assert followers == {"right_lower_eyelid_mimic": "lower_eyelids",
...
def test_visual_limits_are_finite_and_neutral_is_inside_range():
    for joint in model().joints:
        ...
        assert np.all(np.isfinite([joint.limit.lower, joint.limit.upper,
                                  joint.limit.effort, joint.limit.velocity]))
...
def test_rviz_resolves_all_urdf_links_to_the_preview_frame_namespace():
    ...
    link_frames = {f"{prefix}/{link.name}" if prefix else link.name for link in model().links}
