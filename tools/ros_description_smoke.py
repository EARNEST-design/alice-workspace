...
def main():
    ...
    node.create_subscription(JointState, "/alice_preview/joint_states", lambda msg: states.update(msg.name), 10)
    ...
    try:
        while time.monotonic() < deadline:
            ...
            required = ["neck_yaw_link", "left_upper_arm", "left_upper_lid", "right_upper_lid", "left_sole", "head_top"]
            ...
            if (math.isclose(neck.z, math.sin(0.1), abs_tol=1e-5)
                ...
                assert {"neck_rotation", "left_shoulder_pitch", "upper_eyelids"} <= states
                print(f"PASS: {len(states)} joint states, {len(frames)} prefixed TF frames; head, body and mimic transforms update.")
                return
        raise AssertionError(f"Preview transform contract timed out; {len(states)} states, {len(frames)} frames")
    finally:
        node.destroy_node()
        rclpy.shutdown()
